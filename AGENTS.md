...
## Firestore Indexes
- 4 composite indexes on `attendances` (definidos en `firestore.indexes.json`):
  1. (userId ASC, checkInTime DESC) — attendance history queries
  2. (userId ASC, status ASC) — active attendance lookup
  3. (companyId ASC, checkInTime DESC) — company-wide history (supervisor/admin)
  4. (userId ASC, companyId ASC, checkInTime DESC) — history filtrada por empresa
